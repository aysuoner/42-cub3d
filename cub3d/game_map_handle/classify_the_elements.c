...
/*   Updated: 2022/12/02 13:56:47 by aoner            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	classify_the_elements(t_data *data)
{
	...
	i = 0;
	while (data->collect_lines[i] && (!data->no || !data->so || !data->ea || !data->we || !data->f.f || !data->c.f))
	{
		...
		else if (line[0] != '\0') //ya da ft_strlen(line) != 0 yapabilirsin. bir hata olacağını düşünüyorsan bu kullanımın. bazı testleri karşılamayacağını
		{
			free(line);
			return(-1);
		}
